...
import { listCards, updateCard, deleteCard, getAvatarUploadUrl, uploadAvatar } from "../../src/api/cards";
...
export default function EditCard() {
  ...
  const [fields, setFields] = useState({ name: "", title: "", company: "", email: "", phone: "", website: "", bio: "", avatarUrl: "" });
  ...
  useEffect(() => {
    listCards().then((cards) => {
      ...
      if (found) {
        setCard(found);
        setTemplateId(found.templateId as TemplateId);
        setIsDefault(found.isDefault);
        setFields({ name: found.fields.name ?? "", title: found.fields.title ?? "", company: found.fields.company ?? "", email: found.fields.email ?? "", phone: found.fields.phone ?? "", website: found.fields.website ?? "", bio: found.fields.bio ?? "", avatarUrl: found.fields.avatarUrl ?? "" });
      }
      setLoading(false);
    });
  }, [id]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateCard(id, { templateId, isDefault, fields: { ...fields, name: fields.name.trim() } });
      router.back();
    } catch (e: any) {
  ...
  if (!card) return <View style={{ flex: 1 }}><Text>Card not found</Text></View>;
  ...
  return (
    <ScrollView style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.templates}>
        {TEMPLATES.map((t) => (
          <TouchableOpacity key={t.id} style={[styles.templateBtn, templateId === t.id && styles.templateBtnActive]} onPress={() => setTemplateId(t.id)}>
      ...
      <TextInput label="Name *" value={fields.name} onChangeText={set("name")} />
      <TextInput label="Title" value={fields.title} onChangeText={set("title")} />
      <TextInput label="Company" value={fields.company} onChangeText={set("company")} />
      <TextInput label="Email" value={fields.email} onChangeText={set("email")} keyboardType="email-address" autoCapitalize="none" />
      <TextInput label="Phone" value={fields.phone} onChangeText={set("phone")} keyboardType="phone-pad" />
      <TextInput label="Website" value={fields.website} onChangeText={set("website")} autoCapitalize="none" />
      <TextInput label="Bio" value={fields.bio} onChangeText={set("bio")} multiline numberOfLines={3} style={{ height: 80 }} />

      <Button title={fields.avatarUrl ? "Change Photo" : "Add Photo"} variant="ghost" onPress={pickAvatar} style={styles.photoBtn} />
      ...
      <Button title="Delete Card" variant="danger" onPress={handleDelete} />
    </ScrollView>
  );
}
